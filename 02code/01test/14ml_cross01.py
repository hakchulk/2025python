...
# 교차검증
scores = cross_validate(dt, X_train, y_train, cv=5) # cv = k 폴드 교차검증, default:5
print(scores)
# fit_time: 모델 훈련(학습) 시간
# score_time: 평가 시간
# test_score: 평가 점수

print(f'교차검증1 점수 :{np.mean(scores['test_score'])}')
scores = cross_validate(dt, X_train, y_train, cv=StratifiedKFold()) # cv = k 폴드 교차검증
